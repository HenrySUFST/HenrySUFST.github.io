(function () {
	...
	const programControls = function(){
		...
		createNewGraph()
		createDropDown();
		setHeaderImage("Operations");
		//This also sets the tab control to default
		document.getElementById("Spoofing?").addEventListener("mousedown", toggleSpoofing, false);
	}
	...
	function changeButtonColours(SelectedButton){
		...
		for (let i = 0; i < PageButtons.length; i++) {
			...
			if (PageButtons[i].id !== SelectedButton.id){
				...
				PageButtons[i].style.color = "#00bf63";
				headerText.textContent=tabcontent[i].id;
				setHeaderImage(tabcontent[i].id);
			}
		}
		if (SelectedButton.id=="topSettingsBtn"){
			headerText.textContent=tabcontent[3].id;
			setHeaderImage(tabcontent[3].id);
	...
	function setHeaderImage(Image){
		let headerImage = document.getElementById("headerLogo");
		switch (Image) {
			case "Operations":
				headerImage.src="Images/Operation.png";
				break;
			case "Damage":
				headerImage.src="Images/Damage.png";
				break;
			case "Graphs":
				headerImage.src="Images/Graphs.png";
				break;
			case "Settings":
				headerImage.src="Images/Settings.png";		
				break;				
		}
	...
	function changePage(event){
		let selected_button=event.srcElement
		/*This lets the top right button which has no text act as the settings button too
		All other buttons have no image (src)*/
		if (event.srcElement.src != undefined){
			selected_button=document.getElementById("settingsBtn");
		}
		changeButtonColours(selected_button);
	}
	...
	function clickGraph(event){
		...
		const GraphTitle=document.getElementsByClassName("select-selected")[0];
		
		
		if (event.srcElement.id=="Speed"){
			GraphTitle.textContent="Speed Vs Time";
		}
		else{
	...
	const fetchCurrentGraphName = function (){
		return document.getElementsByClassName("select-selected")[0].textContent;
	}
	...
	function createDropDown(){
		...
		const GraphOptions= ["Graph:","Speed Vs Time", "Downtime"]
		...
		for (i = 0; i < l; i++) {
		  ...
		  a.setAttribute("id", "GraphDropdown");
		...
		document.getElementById("GraphDropdown").addEventListener("click", clickDropdown);
	}
